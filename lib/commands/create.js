...
module.exports = function() {
    ...
    return this
        ...
        .arg()
            .name('entities')
            .title('BEM entities to create in form of level/block__elem_mod_val')
            .arr()
            .end()
        .act(function(opts, args) {
            ...
            return Q.all(items.map(function(item) {
                    ...
                    return Q.all(t.map(function(t) {
                            ...
                            if (item.level) {
                                level = typeof item.level == 'string'? require('../level').createLevel(item.level) : item.level;
                            }

                            return context.getTech(t).createByDecl(item, level, addOpts);
                        }))
                        .get(0);
